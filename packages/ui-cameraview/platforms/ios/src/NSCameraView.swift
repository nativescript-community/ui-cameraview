...
public class NSCameraView: UIView, NextLevelVideoDelegate, NextLevelPhotoDelegate, NextLevelDelegate {
    ...
    private func update(zoomMode: Bool) {
        if zoomMode == true {
            if zoomGestureRecognizer == nil {
                let minVideoZoomFactor = self.nextLevel?.minVideoZoomFactor ?? 1.0
                let maxVideoZoomFactor = self.nextLevel?.maxVideoZoomFactor ?? 1.0
                if (minVideoZoomFactor != maxVideoZoomFactor) {
    ...
    @objc func handlePinchToZoomRecognizer(_ gesture: UIPinchGestureRecognizer) {
        let scale = max(min(gesture.scale * pinchScaleOffset, CGFloat(self.maxVideoZoomFactor)), CGFloat(1.0))
        if gesture.state == .ended {
            pinchScaleOffset = scale
            return
        }
        self.nextLevel?.videoZoomFactor = Float(scale)
    }
    ...
    public var zoomEnabled: Bool {
        get {
            return self._zoomEnabled && self.zoomGestureRecognizer != nil
        }
        set {
            self._zoomEnabled = newValue
            self.update(zoomMode: newValue)
        }
    }
    ...
    public var minVideoZoomFactor: Float {
        get {
            return self.nextLevel?.minVideoZoomFactor ?? 1.0
        }
    }
    public var maxVideoZoomFactor: Float {
        get {
            return self.nextLevel?.maxVideoZoomFactor ?? 1.0
        }
    }
    public var neutralVideoZoomFactor: Float {
        get {
            return self.nextLevel?.defaultZoomFactor ?? 1.0
        }
    }
    ...
    public func nextLevelSessionDidStart(_ nextLevel: NextLevel) {
        self.update(zoomMode: self._zoomEnabled)
    ...
    public func nextLevel(_ nextLevel: NextLevel, didUpdateVideoZoomFactor videoZoomFactor: Float) {
        self.delegate?.didUpdateVideoZoomFactor(videoZoomFactor)
    }
